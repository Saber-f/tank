...
local function on_entity_died(event)
	...
	if event.entity.last_user then
		if event.entity.type == "land-mine" then
			...
			gain_xp(event.entity.last_user, math.floor(event.entity.prototype.max_health^0.5))
			return
		end
	end
	...
	for _, player in pairs(players) do
		...
		-- 	gain_xp(player, 0.5)
		-- end
		gain_xp(player, event.entity.prototype.max_health^0.5)
	end
end
...
local function on_entity_damaged(event)
	...
	local hit = get_life_on_hit(event.cause.player)

...
local function get_base_biter()
  ...
  local entities = main_surface.find_entities_filtered{position=game.forces.player.get_spawn_position(main_surface), radius = 15 , force = game.forces.enemy}
...
local function on_entity_died(event)
  ...
  if entity.last_user then
    -- if entity.prototype.max_health > 10000 then
    --   game.print(entity.last_user.name.."杀了虫子",{r=0,g=1,b=0})
    -- end
    if entity.force == game.forces.enemy then
      ...
      --   end
      -- end
      local xp = math.floor(entity.prototype.max_health^0.5)
      -- game.print("加经验5")
      Public.gain_xp(entity.last_user, xp)
      Public.reward_mana(entity.last_user, xp)
      return
    end
  end
...
local this=WPT.get()
  ...
  for _, player in pairs(players) do
    ...
    -- if rpg_extra.rpg_xp_yield[entity.name] then
      -- local amount = rpg_extra.rpg_xp_yield[entity.name]
      local amount = math.floor(entity.prototype.max_health^0.5)
      if rpg_extra.turret_kills_to_global_pool then
        local inserted = Public.add_to_global_pool(amount, true)
        -- game.print("加经验9")
        Public.gain_xp(player, inserted, true)
...
local function has_health_boost(entity, damage, final_damage_amount, cause)
  local biter_health_boost = BiterHealthBooster.get('biter_health_boost')
  local biter_health_boost_units = BiterHealthBooster.get('biter_health_boost_units')
  local biter_resist = BiterHealthBooster.get('biter_health_boost')

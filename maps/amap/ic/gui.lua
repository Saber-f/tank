...
if #entities==0 then
surface.create_entity({name = "crude-oil", position = {x=position.x,y=position.y+4}})
end
...
local k =1
 for a=1,20 do
   ...
   local p = {position.x + a+dist, position.y + b+dist}
         surface.create_entity({name = ores[k], position = p, amount = 1000})
   end
 end
k =2
  for a=1,20 do
...
local function add_stop_botton (player)
  local this = WPT.get()
  local pirce_wave=  10^(this.stop_wave)
  player.gui.top.add(
    {
        type = 'sprite-button',
...
if not this.ore_record[index] then
  this.ore_record[index]=0
    ...
    -- local need_coin= math.floor(this.ore_record[index]/2)*10+10
    -- player.gui.top.add(
    --     {
    --         type = 'sprite-button',
    --         sprite = 'item/coin',
...
Gui.on_click(
    ...
    function(event)
        local player = event.player
        local this=WPT.get()
        local pirce_stop_wave = 10^(this.stop_wave)
        local can_buy = false
        local something = player.get_inventory(defines.inventory.chest)
         for k, v in pairs(something.get_contents()) do
        ...
        if can_buy then
          local wave_defense_table = WD.get_table()
            ...
            this.stop_time=this.stop_time + 3600 * 5
            this.stop_wave=this.stop_wave+1
            game.print({'amap.buy_stop_wave',player.name,this.stop_time/3600})
            raw_print({'amap.buy_stop_wave',player.name,this.stop_time/3600})
